...
dataJSON.forEach((product, i) => {
  ...
  if (product.offer) {
    ...
    productDiv.innerHTML = `<div class="card shadow p-3 my-gradient-color">
      <div class="circleImgContainer my-border-color"><img src="assets/products/${product.image}" class="card-img-top img-fluid max-width-300 max-height-300" alt='${product.name}' /></div>
      <div class="card-body mt-3">
        ...
        <p class="card-text">$ ${
          product.offer
            ...
            : product.price
        } ${product.offer?('<span class="oldPrice">($ '+product.price + ')</span>'):""} <span class="badge no-btn my-badges">${
